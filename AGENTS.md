...
Write tests before implementation. The ticket's Test section specifies the first test — start there. Use `make check-fast` during development, `make check` before opening a PR. Then `/review-pr`, fix all comments, iterate up to 3 times.
