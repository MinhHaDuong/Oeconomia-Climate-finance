...
5. **Update persistent memory** (`$CLAUDE_MEMORY_DIR/MEMORY.md` or equivalent): save durable lessons, then run a stale memory sweep (follow `runbooks/memory.md`; staleness criteria in `docs/memory-policy.md`). For each stale entry: delete it, or archive it with a `[resolved YYYY-MM-DD]` tag if the history matters.
