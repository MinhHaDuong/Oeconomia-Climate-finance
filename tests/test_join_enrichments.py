...
def enrichment_dir(tmp_path):
    ...
    # Language cache — includes ISO 639-3 code "deu" that must be normalized to "de"
    lang = pd.DataFrame({
...
def test_join_applies_language_cache(enrichment_dir):
    ...
    assert ist_row["language"] == "fr", (
        "ISO 639-3 'fra' from cache must be normalized to 'fr'"
    )
    bib_row = result[result["source_id"] == "BIB003"].iloc[0]
    assert bib_row["language"] == "de", (
        "ISO 639-3 'deu' from cache must be normalized to 'de'"
    )
