...
This analysis detects endogenous structural breaks in the temporal evolution of the corpus's thematic composition, without imposing external periodizations (such as COP dates).

### Clustering

KMeans is fitted globally on the full embedding space (not UMAP-reduced) with k=6 and n_init=20 (random_state=42). The minimum sample size per window is N_MIN=30 for the full corpus (20 for the core subset).

### Sliding-window divergence

For each boundary year *y* in the range 2005--2023, and for each window size *w* in {2, 3, 4}:

1. **Before window:** All papers with year in [y-w, y].
2. **After window:** All papers with year in [y+1, y+1+w].
3. **Jensen-Shannon divergence:** Computed on the cluster proportion vectors (k-dimensional probability distributions) of the before and after windows.
4. **Cosine distance:** Computed between the mean embedding vectors of the before and after windows.

Both metrics are z-score normalized across years for each window size, enabling comparison across metrics and windows.

### Breakpoint detection

Local maxima above a z-score threshold of 1.5 are identified as candidate breakpoints. A breakpoint is considered **robust** if it appears (within +/-1 year) in at least 2 of the 3 window sizes. Robust breakpoints from both JS divergence and cosine distance are combined, with breakpoints flagged as "both" when supported by both metrics.

### Volume confound check

Pearson correlation between each divergence series and the year-over-year growth rate of paper counts is computed. Correlations |r| > 0.5 are flagged as potentially confounded by volume growth.

### Results

Two robust structural breaks are detected:
- **2007** (cosine-supported, all 3 window sizes)
- **2013** (JS-supported, all 3 window sizes)

No break is detected near 2015 (Paris) or 2021 (Glasgow). This supports a data-driven three-act periodization: 1990--2006, 2007--2014, 2015--2025.
