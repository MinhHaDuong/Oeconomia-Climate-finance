...
Flagged works are removed unless protected by one of four conditions: citation count $\geq$ 50, presence in 2+ sources, within-corpus citations, or appearance in teaching syllabi. The teaching canon is constructed by an automated scraper that harvests university course syllabi worldwide via DuckDuckGo search and curated seed URLs, classifies pages with an LLM, and extracts bibliographic references using a hybrid strategy: a regular-expression pass captures all explicit DOIs in page text (essential because LLM extraction alone recovers only about a quarter of DOIs present in dense bibliographies), while a second LLM pass extracts title, author, and year for references lacking DOIs. DOIs are cleaned (URL prefixes stripped), and title-only references undergo fuzzy deduplication (rapidfuzz token sort ratio) so that edition variants aggregate their course counts. References without DOIs are enriched via CrossRef title matching, with OpenAlex as a fallback. Users should note a structural harvesting bias: programmes that post syllabi on institutional learning management systems (Moodle, Brightspace, Canvas) rather than public-facing websites are not captured. Every inclusion/exclusion decision is recorded in the audit trail.

### 2.3 Quality, Completeness, and Potential Biases {#sec-quality}

@tbl-quality summarises the corpus composition and metadata completeness per source, computed by `export_corpus_table.py`. The "Refined" column counts all works with a `from_*` flag for that source after deduplication and quality filtering; a single work may appear in multiple rows if discovered by several sources. %DOI is the share of works with a valid DOI. %Abstract is the share with an abstract longer than 50 characters. %Refs is the share with at least one reference in the citation graph (extracted via Crossref and OpenAlex).
...
**Backward compatibility with v1.0.** The `in_v1` boolean column in `refined_works.csv` marks works that were present in the v1.0 submission corpus (git tag `v1.0-submission`, Zenodo DOI [10.5281/zenodo.19097045](https://doi.org/10.5281/zenodo.19097045)). This flag is not a bit-perfect reproduction of v1.0: of the 29,875 v1.0 identifiers, 29,697 are matched in the current corpus (99.4%). The 178 unmatched works reflect genuine pipeline improvements since submission (strengthened quality filters, improved deduplication). Note that `in_v1` filtering does not reproduce the submitted figures exactly: the v1.0 clustering was performed on the original corpus, and re-clustering the `in_v1` subset yields slightly different cluster boundaries. For exact replication of v1.0 results, use the original data from the Zenodo deposit. Researchers needing the exact v1.0 corpus for strict replication can retrieve it from the Zenodo deposit or from the DVC cache using the hash recorded in the `v1.0-submission` tag's lock file (`dvc.lock`, md5 `a905302c`).
...
Reproducibility is ensured at several levels. API queries use fixed year bounds (1990--2024), and raw responses are archived for re-extraction. Deterministic outputs require `PYTHONHASHSEED=0` and `SOURCE_DATE_EPOCH=0` (set automatically by the Makefile). The four automated or hybrid-automated sources (OpenAlex, ISTEX, grey literature, teaching canon) can be re-harvested from the scripts with an internet connection. The two restricted sources (bibCNRS, SciSpace) have their raw exports included in the Zenodo deposit to ensure full reproducibility despite access restrictions.
