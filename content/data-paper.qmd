...
**Backward compatibility with v1.0.** The `in_v1` boolean column in `refined_works.csv` marks works that were present in the v1.0 submission corpus (git tag `v1.0-submission`, Zenodo DOI [10.5281/zenodo.19097045](https://doi.org/10.5281/zenodo.19097045)). This flag is not a bit-perfect reproduction of v1.0: of the 29,878 v1.0 rows, 29,696 are matched in the current corpus (99.4%). The 182 unmatched rows reflect genuine pipeline improvements since submission: 107 were removed by strengthened quality filters that now correctly flag off-topic works (e.g., pattern recognition, fiber composites) that v1.0 had inadvertently retained, and 75 were absorbed by improved deduplication and DOI normalization. In all cases, the v2 corpus is more accurate than v1.0---the missing rows are noise, not signal. Researchers needing the exact v1.0 corpus for strict replication can retrieve it from the Zenodo deposit or from the DVC cache using the hash recorded in the `v1.0-submission` tag's lock file (`dvc.lock`, md5 `a905302c`).
