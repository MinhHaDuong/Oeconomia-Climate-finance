...
Flagged works are removed unless protected by one of four conditions: citation count $\geq$ 50, presence in 2+ sources, within-corpus citations, or appearance in 2+ teaching syllabi. The teaching canon is constructed by an automated scraper that harvests university course syllabi worldwide via DuckDuckGo search and curated seed URLs, classifies pages with an LLM, and extracts bibliographic references. DOIs are cleaned (URL prefixes stripped) and title-only references undergo fuzzy deduplication so that edition variants aggregate their course counts. A two-tier convergence filter distinguishes detailed syllabi (doctoral seminars with 20+ DOI readings, passing at one course) from standard readings (requiring cross-course corroboration). Every inclusion/exclusion decision is recorded in the audit trail.
