...
@tbl-qa summarises the corpus composition and quality metrics per source, computed by automated QA scripts.
...
| **Total (deduplicated)** | **29,878** | **72%** | **86%** | **41%** | **78%** |

: Per-source corpus quality metrics. Refined counts exceed raw for small sources because deduplication assigns multi-source works to all contributing sources. Generated by `export_corpus_table.py`. {#tbl-qa}
...
Relevance filtering (flag 6) uses a cross-encoder reranker (BAAI/bge-reranker-v2-m3, 568M parameters) rather than a generative LLM, ensuring deterministic, reproducible classification without API costs. The model was calibrated on weak labels derived from independent corpus signals. Human validation on a stratified sample of 100 works (20 per score quintile, blinded to scores) yielded AUC = 0.818 and 81% accuracy (precision 74%, recall 76%). An independent LLM audit (Gemma 2 27B) provides a second check. Both methods serve as independent checks on the flagging pipeline, not as ground truth, since what counts as "climate finance" is itself a contested category.
