...
The meaningful layer is G9. Its supra-threshold years at $w = 3$ cluster into {{< meta g9_n_zones >}} contiguous runs ({{< meta g9_zones_listing >}}), each a candidate locus of community-structure reorganisation in the co-citation graph. C2ST AUC rises monotonically across the analysis window on both channels, consistent with accumulating distributional drift but not with point discontinuities.
...
The discriminative-terms and community-shift interpretation outputs (§4.9) require `compute_interpretation.py` to run per zone; its CSVs are absent from the ticket-0042 rerun and are scheduled under ticket 0056. The placeholders below remain unresolved pending that output. When the follow-up lands, @fig-terms and @fig-community will show the concrete shifts.

For zone 1 ({{< meta zone_1_start >}}--{{< meta zone_1_end >}}): terms gaining weight, {{< meta zone_1_top_terms_after >}}; terms losing weight, {{< meta zone_1_top_terms_before >}}. Community shift: {{< meta zone_1_community_before >}} to {{< meta zone_1_community_after >}}. For the G9 sub-zones the same procedure will run per run listed in §5.2.
