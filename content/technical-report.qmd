...
| **Reference / sanity check** | C2ST | Classifier accuracy sits at 0.60–0.67 throughout — above chance in every year. Confirms that the distance-based methods are not measuring noise; does not vote on zones. |
...
The test statistic is the cross-validated classifier accuracy (or AUC); the null is $\alpha = 0.5$.
...
*C2ST principle: a logistic classifier fits a decision boundary in PCA-projected embedding space; cross-validated accuracy above 0.50 confirms the before/after distributions differ. Script: `scripts/plot_schematic_C2ST.py`.*
...
**Biases.** Accuracy depends on classifier capacity:
...
its accuracy has a different sampling distribution and cannot be fused into the same Z-score scale without rescaling.
...
accuracy visibly above 0.5 corroborates the distance-based signals but does not by itself validate a zone.
