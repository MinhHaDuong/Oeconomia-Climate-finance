// ticket-tools — validate, ready, archive %ticket v1 files.
...
// ---------------------------------------------------------------------------
// Ticket parser — %ticket v1 format
// ---------------------------------------------------------------------------

const magicLine = "%ticket v1"
...
// FilenameID extracts the numeric prefix from the filename (e.g., "0042" from "0042-add-auth.ticket").
...
var (
	...
	filenameRE = regexp.MustCompile(`^\d{4}-[a-z0-9]+(-[a-z0-9]+)*\.ticket$`)
...
func validateTicket(t *Ticket, allIDs map[string]bool) []string {
	...
	if !t.HasMagic {
		errors = append(errors, fmt.Sprintf("%s: missing magic first line '%%ticket v1'", name))
	}
	...
	if !filenameRE.MatchString(name) {
		errors = append(errors, fmt.Sprintf(
			"%s: filename does not match NNNN-slug.ticket pattern", name))
	}
...
func cmdArchive(args []string) int {
	...
	for i := range allTickets {
		for _, ref := range allTickets[i].BlockedBy() {
			if !strings.HasPrefix(ref, "gh#") {
...
func printUsage() {
	...
	fmt.Fprintln(os.Stderr, "  validate [dir|files...]   Validate %ticket v1 files")
