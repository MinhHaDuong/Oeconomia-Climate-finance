...
def _node_permutation_null_distribution(
    ...
    Used by G2, G5, G6, G8 — any method whose statistic is
    abs(metric(G_after) - metric(G_before)) for a scalar graph metric.

    Parameters
    ----------
    G_union : networkx.DiGraph
        Union graph containing all nodes from before + after windows.
    all_nodes : list
        Ordered list of all node IDs (before nodes first, then after nodes).
    n_before : int
        Number of before-window nodes.
    n_perm : int
        Number of permutations.
    perm_rng : np.random.RandomState
        Per-window RNG for reproducibility.
    metric_fn : callable(G_subgraph) -> float
        Scalar graph metric. Should return np.nan on failure.

    Returns
    -------
    np.ndarray
        Array of abs-diff values with NaN entries removed.
...
def _build_union_digraph(G_before, G_after, internal_edges):
    ...
    Unlike _build_union_graph (which returns an undirected Graph for
    community detection), this preserves edge direction so that in-degree
    metrics (entropy, PA exponent, betweenness) work correctly on subgraphs.
    """
...
def _run_g8_permutations(works, internal_edges, div_df, cfg, n_jobs=1):
    ...
    n_perm is capped at 50 regardless of config because computing
    betweenness centrality inside a permutation loop is O(n*m) and
    prohibitively slow at n_perm=500.  The approximation (k=max_nodes
    sampling inside _mean_betweenness) introduces non-determinism but
    is consistent between observed and null statistics.
    """
