...
def _is_missing(val):
    """Return True if a string value is empty/missing/NaN."""
    if pd.isna(val):
        return True
    s = str(val).strip()
...
def _load_csv_cache(path, key_col, val_col):
    ...
    try:
        ...
        return {}
    return dict(zip(df[key_col].astype(str), df[val_col], strict=False))
...
def _apply_language_cache(df, cache_dir):
    ...
    for idx in df.index:
        if _is_missing(df.at[idx, "language"]):
            doi = normalize_doi(df.at[idx, "doi"])
            sid = str(df.at[idx, "source_id"])
            raw = lang_cache.get(doi, "") if doi else ""
            if not raw:
                raw = lang_cache.get(sid, "")
            if raw:
...
def main():
    """Parse CLI arguments and run the join."""
