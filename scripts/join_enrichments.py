...
def join_enrichments(unified_path, output_path, cache_dir=None):
    ...
    for idx in df.index:
        if _is_missing(df.at[idx, "doi"]):
            ...
            resolved = doi_cache.get(sid, "")
            if resolved:
                df.at[idx, "doi"] = resolved
                doi_applied += 1
    log.info("DOI cache: applied %d (cache has %d entries)", doi_applied, len(doi_cache))

    # --- 2. Cross-source abstract backfill ---
    # Records with the same DOI may have abstracts from different sources.
    doi_abstract = {}
    for idx in df.index:
        doi = normalize_doi(df.at[idx, "doi"])
        abstract = df.at[idx, "abstract"]
        if doi and not _is_missing(abstract):
            doi_abstract.setdefault(doi, abstract)
    crossfill = 0
    for idx in df.index:
        if _is_missing(df.at[idx, "abstract"]):
            doi = normalize_doi(df.at[idx, "doi"])
            if doi and doi in doi_abstract:
                df.at[idx, "abstract"] = doi_abstract[doi]
                crossfill += 1
    log.info("Cross-source backfill: %d abstracts", crossfill)

    # --- 3. Apply abstract caches (OpenAlex, Semantic Scholar) ---
    oa_cache = _load_csv_cache(
        os.path.join(cache_dir, "openalex_abstracts.csv"),
        "key", "abstract",
    )
    s2_cache = _load_csv_cache(
        os.path.join(cache_dir, "s2_abstracts.csv"),
        "key", "abstract",
    )
    abs_applied = 0
    for idx in df.index:
        if _is_missing(df.at[idx, "abstract"]):
            sid = str(df.at[idx, "source_id"])
            doi = normalize_doi(df.at[idx, "doi"])
            # Try OpenAlex cache (keyed by source_id)
            val = oa_cache.get(sid, "")
            if not val and doi:
                # Try S2 cache (keyed by DOI)
                val = s2_cache.get(doi, "")
            if val:
                df.at[idx, "abstract"] = val
                abs_applied += 1
    log.info("Abstract caches: applied %d (OA=%d, S2=%d entries)",
             abs_applied, len(oa_cache), len(s2_cache))

    # --- 4. Normalize language + apply language cache ---
    df["language"] = df["language"].apply(normalize_lang)
    lang_cache = _load_csv_cache(
        os.path.join(cache_dir, "language_resolved.csv"),
        "key", "language",
    )
    lang_applied = 0
    for idx in df.index:
    ...
    log.info("Language cache: applied %d (cache has %d entries)",
             lang_applied, len(lang_cache))

    # --- 5. Apply abstract summaries + classify abstract_status ---
    summary_cache = _load_summary_cache(
        os.path.join(cache_dir, "abstract_summaries_cache.jsonl"),
    )
    TOKEN_LIMIT = 1000

    def classify_status(row_idx):
        abstract = df.at[row_idx, "abstract"]
        if _is_missing(abstract):
            return "missing"
        if _count_tokens(abstract) > TOKEN_LIMIT:
            doi = normalize_doi(df.at[row_idx, "doi"])
            if doi and doi in summary_cache:
                entry = summary_cache[doi]
                summary = entry.get("summary", "")
                error = entry.get("error")
                if summary and not error:
                    df.at[row_idx, "abstract"] = summary
                    return "generated"
            return "too_long"
        return "original"
    ...
    statuses = []
    for idx in df.index:
        statuses.append(classify_status(idx))
