...
print("=" * 70)
print(f"DETECTING PRE-{CUTOFF_YEAR + 1} INTELLECTUAL TRADITIONS")
print(f"(Co-citation network, top {TOP_N} most-cited references)")
print("=" * 70)
...
print(f"Works with year <= {CUTOFF_YEAR}: {len(pre_cutoff)}")
...
print(f"Added metadata for {ref_meta_added} references not in refined_works")
...
print(f"Pre-{CUTOFF_YEAR + 1} corpus DOIs: {len(pre_dois)}")

source_dois = set(cit["source_doi"])
pre_as_sources = pre_dois & source_dois
print(f"  Appearing as citation sources: {len(pre_as_sources)}")

ref_dois = set(cit["ref_doi"])
pre_as_refs = pre_dois & ref_dois
print(f"  Appearing as cited references: {len(pre_as_refs)}")
...
print(f"  Of which in corpus: {len(pre_refs_in_works)}")
...
print("\n" + "=" * 70)
print(f"CO-CITATION NETWORK: top {TOP_N} most-cited pre-{CUTOFF_YEAR + 1} references")
print("(Two refs linked if co-cited by ANY paper in the corpus)")
print("=" * 70)
...
print(f"  cited >= 3 times: {(pre_ref_counts >= 3).sum()}")
print(f"  cited >= 5 times: {(pre_ref_counts >= 5).sum()}")
print(f"  cited >= 10 times: {(pre_ref_counts >= 10).sum()}")
...
print(f"Min citations in top set: {min_cit_in_top}")
    ...
    for c in sorted(comm_papers.keys()):
        ...
        if len(texts) >= 3:
            try:
                ...
                print(f"  Top TF-IDF terms ({len(texts)} texts): {top_terms_str}")
...
print("\n" + "=" * 70)
print("DEFAULT RESOLUTION (gamma=1.0)")
print("=" * 70)
...
print(f"Communities detected: {n_comm} (sizes: {size_str})")
...
for gamma in [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.2, 1.5, 2.0]:
    ...
    for v in part.values():
        sizes[v] += 1
    size_str = ", ".join(f"{s}" for s in sorted(sizes.values(), reverse=True))
    print(f"  gamma={gamma:.1f}: {n_c} communities (sizes: {size_str})")
...
print("\n" + "=" * 70)
print("FINER RESOLUTION (gamma=0.5)")
print("=" * 70)
...
print(f"Communities detected: {n_comm_fine} (sizes: {size_str_fine})")
