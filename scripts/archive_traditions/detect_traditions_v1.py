...
# =============================================================================
print("=" * 70)
print("DETECTING PRE-2007 INTELLECTUAL TRADITIONS VIA SEMANTIC CLUSTERING")
print("=" * 70)
...
print(f"Loaded {len(df_all)} papers with {embeddings_all.shape[1]}D embeddings")
...
# =============================================================================
print("\n" + "=" * 70)
print("KMEANS CLUSTERING (k=3, 4, 5)")
print("=" * 70)
...
for k in [3, 4, 5]:
    ...
    for c, n in sizes.items():
        print(f"  Cluster {c}: {n} papers ({100*n/len(df):.1f}%)")
...
# =============================================================================
print("\n" + "=" * 70)
print("TOP-CITED PAPERS PER CLUSTER")
print("=" * 70)
...
for k in [3, 4, 5]:
    ...
    for c in range(k):
        ...
        for _, row in cluster_papers.iterrows():
            ...
            print(f"    [{cites:>5} cites] {author} ({yr}) {title}")

# =============================================================================
# 6. Silhouette comparison and recommendation
# =============================================================================
print("\n" + "=" * 70)
print("SUMMARY: SILHOUETTE SCORES")
print("=" * 70)
for k in [3, 4, 5]:
...
# =============================================================================
print("\n" + "=" * 70)
print("YEAR DISTRIBUTION PER CLUSTER (k=3)")
print("=" * 70)

labels_3 = results[3]["labels"]
for c in range(3):
    years = df.loc[labels_3 == c, "year"]
    print(f"  Cluster {c}: mean year={years.mean():.1f}, "
...
# =============================================================================
print("\n" + "=" * 70)
print("LANGUAGE DISTRIBUTION PER CLUSTER (k=3)")
print("=" * 70)
...
# =============================================================================
print("\n" + "=" * 70)
print("SOURCE DISTRIBUTION PER CLUSTER (k=3)")
print("=" * 70)
...
if len(df_core) >= 10:
    ...
    print(f"Silhouette (core, k=3): {sil_core:.3f}")
    ...
    for c in range(3):
        ...
        for _, row in cluster_papers.iterrows():
            ...
            yr = int(row["year"]) if pd.notna(row["year"]) else "?"
            cites = int(row["cited_by_count"])
            print(f"    [{cites:>5}] {author} ({yr}) {title}")

    df_core["cluster_core_k3"] = labels_core
else:
    print("Too few core papers for clustering.")
...
print(f"Saved {len(out_clusters)} rows -> content/tables/traditions_clusters.csv")
...
print(f"Saved {len(top_papers_df)} rows -> content/tables/traditions_top_papers.csv")
...
print(f"Saved {len(terms_df)} rows -> content/tables/traditions_top_terms.csv")
