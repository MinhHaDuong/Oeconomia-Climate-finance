...
=======
| `analyze_bimodality.py` | `tab_bimodality.csv`, `tab_axis_detection.csv`, `tab_pole_papers.csv` (+ core variants) | acceptable | Tables-only after #550 split. Three figures moved to `plot_bimodality.py`, `plot_bimodality_lexical.py`, `plot_bimodality_keywords.py`. |
...
>>>>>>> f5ac24b (refactor: split analyze_bimodality.py into compute + 3 plot scripts (#550))
